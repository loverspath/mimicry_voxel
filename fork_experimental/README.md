...
### 2. ⚔️ 나노바나나 전투 VFX 엔진 (`CombatVFXEngine.js`)
- **실전 텍스처 에셋 결합**:
  - `fx_slash.jpg`: 은빛 검기 슬래시 아크 & 절단 스파크 방사
  - `fx_fire.jpg`: 작열하는 화염 폭발 구체
  - `fx_frost.jpg`: 청백색 빙결 파편 비산
- **1인칭 스크린 스페이스 슬래시 & 월드 빌보드 폭발**: 플레이어 공격 시 화면을 가로지르는 베지어 참격 궤적, 몬스터 위치 중심의 3D 폭발 연출.
- **화면 셰이크(Screen Shake) & 핏빛 비네팅(Blood Vignette)**: 치명타 타격감 및 피격 시 가장자리 붉은 섬광 펄스.
